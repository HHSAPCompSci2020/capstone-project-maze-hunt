...
People who would want to use our program are those that like to problem solve and challenge themselves through different riddles and/or puzzles. Our game presents challenging clues that get harder with each clue as the player progresses through. The primary features of our game are that there are hidden clues on the maze that need to be solved throughout the maze, the player (mouse) can pick up “diamonds” along the way that can be used later on to slow down the cat when trying to solve a clue or navigate through the maze to prevent themselves from being eaten up. The player (mouse) can also pick up super boosts that they find on the maze that makes them speed up and go faster so that they can run away faster from the cat. More features include pop up windows to solve the clues and also integrating the clus into the actual maze and not just having obvious circles on the grid.




Instructions: 
The cat will be controlled using the WASD keys on the keyboard and the mouse will be controlled using the arrow keys on the same keyboard. Places you will need to click include the startup menu to start the game, a clue on the maze to start solving the clue, buttons that will appear on the pop up windows to allow the player to progress or choose different actions against the cat, and diamonds that are present on the maze for the mouse to slow down the cat. In the beginning of the game a startup window will appear asking players welcoming players to the game and then they will be directed to choose which character they want to be. There will also be a help button that leads to the window of how to play the game and the different rules.Once the players have chosen their characters, they will then press a button to start the game. Once the player starts the game, the players will start navigating through the maze. Once the mouse arrives at a clue another pop up window will appear directing the mouse to solve whatever riddle or puzzle that it has and then the clue for where the next clue is will be revealed and displayed. Once the mouse has figured out and solved all the clues, the exit of the maze will open, otherwise the exit of the maze is always closed and players do not know where it is. Actions do need to be taken in certain order in the sense that players need to first enter through the welcome screen and then choose characters, then enter the game to start playing, and the mouse has to solve the clues in order.
...
* The clues will be integrated to be part of the actual maze and not random circles lying around. When the player solves the riddle/puzzle that reveals the where the next clue is, the next clue will be lit up/change color on the maze to show where the player should go. The exit will also not be revealed until the very last clue is found, once the last clue is solved the exit will actually be opened and once the player exits the game a end game pop up screen will appear summarizing the player’s journey throughout the maze and their achievements.
...
* Help Class, where the instructions for the game and rules are located
* Maze class, where the maze is to be constructed and built
* Player 1 class(mouse), where player 1’s entirety will be coded ranging from the construction of the player based on what the user picks in the beginning, to the player's movements, to its actions.
* Player 2 class(cat), where player 2’s entirety will be coded ranging from the construction of the player based on what the user picks in the beginning , to the player's movements, to its actions.
* Superboost class, where the speed of the mouse is altered depending on when the superboost is activated. 
* Sounds class, where different sounds are played for different actions made by the players
* Diamond class, where the speed of the cat is altered depending on when the diamond is activated against the cat
* Clues class, this class will be the superclass and have multiple subclasses to represent the different clues that will be displayed on the screen and their respective puzzle/riddle that they need to solve before the answer to the clue is revealed
